...
.provider('terminalConfiguration', function () {
    ...
    var config = function () {
        ...
        me.promptConfiguration = { end: ':>', user: 'infotsav', separator: '@', path: '\\' };
...
.service('promptCreator', [function () {
    var prompt = function (config) {
        ...
        me.resetUser = function () {
            _user = config && config.user ? config.user : 'infotsav';
            build();
        }

        me.reset = function () {
            _user = config && config.user!=null ? (config.user||'') : 'infotsav';
